# -*- coding: utf-8 -*-
"""CODEX + WORKBUDDY Token 实时监控 · HTTP 服务 (v2)
   改动：WEB_DIR 指向 web_v2；build_payload 合并「我发送的最新信息」原文。
    python server_v2.py [--port 8910] [--interval 3]
"""
...
def build_payload() -> dict:
    ...
    p["paused"] = PAUSED
    try:
        um = extras_v3.get_last_user_messages()
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path in ("/", "/index.html"):
            try:
                body = (WEB_DIR / "_edited.html").read_bytes()
